...
const modifyPatientEntry = (object: PatientEntry, id: string): PatientEntry => {
  const newPatientEntry = object;
  for (let i = 0; i < patientEntries.length; i++) {
    if (patientEntries[i].id === id) {
      patientEntries.splice(i, 1)
      patientEntries.push(newPatientEntry);
    } 
  }
